...
in {
  ...
  programs = {
    ...
    foot = lib.mkIf (stdenv.isLinux && osConfig.services.xserver.enable) {
      enable = true;
    };
  ...
  gtk = lib.mkIf (stdenv.isLinux && osConfig.services.xserver.enable) {
    enable = true;
    iconTheme = {
  ...
  #   ''
  dconf.settings = lib.mkIf (stdenv.isLinux && osConfig.services.xserver.enable) {
    "org/gnome/shell" = {
