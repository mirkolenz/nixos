        ...
        lib.foldl lib.recursiveUpdate
          ...
          [
            ...
            (lib.optionalAttrs (network.driver == "bridge" && network.defaultRoute != null) {
              options.no_default_route = if network.defaultRoute then "false" else "true";
            })
          ];
    in
    lib.mkIf network.enable (json.generate "${name}.json" networkConfig);
in
{
  options.custom.oci.networks =
    with lib;
    mkOption {
      default = { };
      type = types.attrsOf (
        types.submodule {
          options = {
            ...
            prefix = mkOption { type = with types; str; };
            id = mkOption { type = with types; str; };
            interface = mkOption { type = with types; str; };
            subnets = mkOption {
