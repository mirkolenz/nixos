...
rec {
  mkOptions =
    let
      ...
      mkNestedAttrsOptionValuePair = { name, value }: "${name}:${concatAttrsOptionValue value}";

      mkAttrsOptionValuePair = { name, value }: "${name}=${mkDefaultValue value}";

      mkAttrsOptionValue =
        attrs:
        if isNameValuePair attrs && attrs.value == null then
          attrs.name
        else if isNameValuePair attrs && builtins.isAttrs attrs.value then
          mkNestedAttrsOptionValuePair attrs
        else if isNameValuePair attrs then
          mkAttrsOptionValuePair attrs
        else
          concatAttrsOptionValue attrs;
  ...
  mkVolumes =
    let
      mkAttrVolume =
        ...
        if mode == null then "${source}:${target}" else "${source}:${target}:${mode}";
  ...
  mkSysctls =
    attrs:
    mkOptions {
      sysctl = lib.attrsToList (
        lib'.flocken.getLeaves (
          {
  ...
  mkEnv = attrs: mkOptions { env = lib.attrsToList ({ TZ = "Europe/Berlin"; } // attrs); };
  ...
  mkImage =
    image:
    if builtins.isString image then
      image
    else if image.registry == null then
